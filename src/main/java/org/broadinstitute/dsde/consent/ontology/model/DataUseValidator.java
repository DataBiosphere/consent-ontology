...
public class DataUseValidator {
  ...
  /**
   * Other use is enforced in json-schema. Here for an OO approach. If other is specified, we need
   * one of the sub-conditions to be present as well.
   *
  ...
  /**
   * Recontact use is enforced in json-schema. Here for an OO approach. If recontacting is allowed,
   * we need to have some condition for that.
   *
   * @param dataUse The data use to check
   * @return Boolean: valid or not
   */
