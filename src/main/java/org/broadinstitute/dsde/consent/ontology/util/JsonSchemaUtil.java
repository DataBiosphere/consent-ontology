  ...
  public List<String> validateDataUseV4Schema(String dataUseV4Instance) {
    try {
      ...
      if (diseases != null && diseases.isArray()) {
        List<JsonNode> diseaseNodes = StreamSupport.stream(diseases.spliterator(), false).toList();
        if (!diseaseNodes.isEmpty()) {
          diseasePositive = true;
        }
      }
      List<Boolean> primaryPositives =
          Stream.of(gruPositive, hmbPositive, diseasePositive).filter(p -> p).toList();
      if (primaryPositives.isEmpty()) {
        // Note that error messages here are formatted to resemble the messages generated by the
        // schema validator
        List<String> required =
            List.of(
                "$: At least one of 'generalUse', 'hmbResearch' must be true or 'diseaseRestrictions' cannot be empty");
        return Stream.concat(validationMessages.stream(), required.stream()).toList();
      }
      if (primaryPositives.size() > 1) {
        List<String> tooManyPrimaries =
            List.of(
                "$: At most, only one of the following conditions may be true: 'generalUse'; 'hmbResearch'; 'diseaseRestrictions' cannot be empty");
        return Stream.concat(validationMessages.stream(), tooManyPrimaries.stream()).toList();
      }
      return messages.stream().map(ValidationMessage::getMessage).toList();
    } catch (Exception e) {
      logWarn("Unable to load the data use schema: " + e.getMessage());
