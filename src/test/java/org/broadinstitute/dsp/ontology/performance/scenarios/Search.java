...
public class Search implements Endpoints {
  ...
  public List<ScenarioBuilder> tests = List.of(
    scenario("Search: DOID_162").exec(search("DOID_162").check(status().is(HttpStatusCodes.STATUS_CODE_OK))).pause(1, 5),
    scenario("Search: DOID_4").exec(search("DOID_4").check(status().is(HttpStatusCodes.STATUS_CODE_OK))).pause(1, 5)
  );
